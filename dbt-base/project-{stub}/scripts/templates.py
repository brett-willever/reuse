common = """{% macro generate_base_model_query(input_reference, columns) %}
WITH 
ab1 AS (
    SELECT
        {% for column in columns %}
            CASE WHEN SAFE_CAST({{column}} AS STRING) = '*' THEN CAST('0' AS STRING) ELSE CAST({{column}} AS STRING) END AS {{column}}{% if not loop.last %},{% endif %}
        {% endfor %},
        MD5(CAST(
            {% for column in columns %}
                COALESCE(CAST({{ column }} AS STRING), ''){% if not loop.last %} || '-' ||{% endif %}
            {% endfor %}
        AS STRING)) AS _dvp_id,
...
ab2 AS (
    SELECT
        {% for column in columns %}
            CAST({{ column }} AS STRING) AS {{column }}_casted{% if not loop.last %},{% endif %}
        {% endfor %},
        _dvp_emitted_at
    FROM ab1
),
...
WITH union_tables AS (
  {% for config in table_configs %}
    ...
    SELECT
      ...
      CASE WHEN SAFE_CAST({{column}} AS STRING)= '*' THEN CAST('0' AS {{type}}) ELSE CAST({{column}} AS {{type}}) END AS {{column}}{% if not loop.last %},{% endif %}
...
SELECT
  {% for column, data_type in columns %}
    CAST({{ column }} AS {{ data_type }}) AS {{ column }}{% if not loop.last %},{% endif %}
  {% endfor %}
FROM
...
{% macro char_replace_cast(character, replacement, source, target, data_type) %}
  CAST(IF(SAFE_CAST({{source}} AS STRING)= '{{character}}', '{{replacement}}', {{source}}) AS {{data_type}}) AS {{target}}
{% endmacro %}
...
{% macro generate_surrogate_key(input_key) %}
  ABS(FARM_FINGERPRINT(SAFE_CAST('{{input_key}}' AS STRING)))
{% endmacro %}"""
...
snapshot_template = """
{% macro snapshot_template(target, dataset, model) %}
    {% snapshot {{model}}_snapshot %}
      {{ config(
      target_schema='{{target}}',
      materialized='snapshot',
      unique_key='_dvp_id',
      snapshot_timestamp='valid_to',
      strategy='timestamp',
      updated_at='valid_to'
      ) }}
    ...
    FROM
    {{ source({{dataset}}.{{model}}) }} br
    WHERE
    br.valid_to IS NULL 
    {% endsnapshot %}"""


macros = common + header_common + scd_model
